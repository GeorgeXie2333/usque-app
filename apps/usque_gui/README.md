...
- Chain proxy places the enable switch above the source choices (OpenVPN, WireGuard, VPN Gate). The choices wrap on wide layouts and stack one per line below 600 logical pixels or at large text; the selected choice carries its check after its name, never over its icon. Switching sources never prompts: drafts made under one source are retained until settings are applied, and only leaving the page asks about unapplied edits. One saved-configuration list carries the saved/current/CONNECT-IP markers, and the action bar names the pending selection and the reason a draft cannot be applied. Connection failures and missing DNS use inline warning banners. Chain copy is localized in every catalog.
- Home uses open sections: the connection ring and Kill Switch state, shared upload/download traces, and a connection overview with quality/diagnostic actions. Desktop places the connection control beside the status/location readouts; narrow content or text above 150% stacks them. Connected sessions show exit region, duration, and protocol; idle/error sessions explain the configured outputs. On phones, full addresses and output runtime details remain under **Connection details**.
