...
fn should_use_cmake_cross_compilation(config: &Config) -> bool {
    ...
    match config.target_os.as_str() {
        // Android's NDK CMake toolchain selects an API-qualified compiler
        // target (for example aarch64-none-linux-android26). Passing Cargo's
        // unqualified Rust target as CMAKE_<LANG>_COMPILER_TARGET overrides
        // that selection and makes Clang unable to find crtbegin_dynamic.o.
        // The NDK toolchain already enables CMake cross compilation.
        "android" => false,
        "macos" | "ios" => {
    ...
    match &*config.target_os {
        "android" => {
            ...
            boringssl_cmake
                ...
                // cmake-rs otherwise copies cc-rs' API-unqualified Rust
                // target into these variables. Defining the flags here keeps
                // the NDK toolchain's API-qualified compiler target intact.
                .define("CMAKE_C_FLAGS", "-w")
                .define("CMAKE_CXX_FLAGS", "-w")
                .define("CMAKE_ASM_FLAGS", "-w");
...
fn get_extra_clang_args_for_bindgen(config: &Config) -> Vec<String> {
    ...
    match &*config.target_os {
        ...
        "windows" => {
            // libclang does not inherit MSVC's INCLUDE search path when it is
            // loaded through bindgen. The native compiler environment created
            // by vcvars64.bat is authoritative, so forward each existing SDK
            // and CRT include directory explicitly. Without this, clean
            // release builds fail on BoringSSL's <sys/types.h> even though the
            // same headers compile correctly with cl.exe.
            println!("cargo:rerun-if-env-changed=INCLUDE");
            params.push(format!("--target={}", config.target));
            if let Some(include) = env::var_os("INCLUDE") {
                for path in env::split_paths(&include).filter(|path| path.is_dir()) {
                    params.push(format!("-I{}", path.display()));
                }
            }
        }
        _ => {}
