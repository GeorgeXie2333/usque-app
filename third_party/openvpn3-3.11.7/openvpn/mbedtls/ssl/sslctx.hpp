  ...
  public:
    ...
    class Config : public SSLConfigAPI
    {
      ...
      public:
        ...
        Config()
            : external_pki(nullptr),
              ...
              tls_version_max(TLSVersion::Type::V1_3),
        ...
        void load(const OptionList &opt, const unsigned int lflags) override
        {
            ...
            // parse tls-version-min option
            {
                ...
                const TLSVersion::Type maxver = TLSVersion::Type::V1_3;
#elif defined(MBEDTLS_SSL_MAJOR_VERSION_3) && defined(MBEDTLS_SSL_MINOR_VERSION_3)
                ...
                if (const auto *maximum = opt.get_ptr(relay_prefix + "tls-version-max"))
                    tls_version_max = TLSVersion::parse_tls_version_min(maximum->get(1, 16), false, maxver);
            }
      ...
      private:
        ...
        TLSVersion::Type tls_version_max; // maximum TLS version that we will negotiate
      ...
      protected:
        SSL(MbedTLSContext *ctx, const char *hostname)
        {
            ...
            try
            {
                ...
                if (c.tls_version_max == TLSVersion::Type::V1_2)
                    mbedtls_ssl_conf_max_tls_version(sslconf, MBEDTLS_SSL_VERSION_TLS1_2);
                else if (c.tls_version_max == TLSVersion::Type::V1_3)
    ...
    /* This is basically is the same as tls-version-max that OpenVPN 2.x has but hardcoded */
	    mbedtls_ssl_conf_max_version(sslconf, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
                ...
                {
                    // OpenVPN authenticates through its CA/KU/EKU and
                    // verify-x509-name callback, not HTTPS DNS SAN matching.
                    // Mbed TLS 3.6 requires this explicit choice. Keep the
                    // callback and VERIFY_REQUIRED mode unchanged.
                    if (mbedtls_ssl_set_hostname(ssl, nullptr))
                        throw MbedTLSException("mbedtls_ssl_set_hostname failed");
                }
