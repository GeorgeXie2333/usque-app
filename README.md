...
- The release workflow compiles and signs the six packages, then attaches them with `release-manifest.json`, `SHA256SUMS`, per-package SPDX SBOMs, `reliability-report.json`, and `device-matrix.md`. The exact signed candidate must also produce accepted reports from the protected Windows snapshot VM, Android device, independent network observer, and controlled performance lab; restricted captures and raw lab evidence remain CI-only.
