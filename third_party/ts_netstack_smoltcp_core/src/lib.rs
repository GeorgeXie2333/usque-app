...
impl Netstack {
    ...
    fn pump_blocked_commands(&mut self) {
        ...
        for _ in 0..self.blocked_commands.len() {
            ...
            // A cancelled async operation drops its response receiver. Replaying
            // that operation is both useless and unsafe: the socket's queued
            // Close command may already have removed the smoltcp handle.
