import 'app_strings.dart';

// All supported catalogs carry the same compact scanner vocabulary. Shared
// actions (cancel, reset, port) continue to use the application's main catalog.
const _keys = [
  'generate',
  'scan',
  'quick',
  'target',
  'full',
  'pause',
  'resume',
  'country',
  'history',
  'more',
  'observed',
  'unknown',
  'running',
  'paused',
  'completed',
  'cancelled',
  'failed',
  'all',
  'stale',
  'unavailable',
  'hint',
  'full_hint',
  'identity_required',
  'underlay_failed',
  'registration_failed',
  'single_port_hint',
  'scan_plan_changed',
];
const kWarpCatalogs = <String, List<String>>{
  'en': [
    'Generate WARP configuration',
    'Scan endpoints',
    'Quick scan',
    'Scan one IP',
    'Complete IPv4 pool',
    'Pause',
    'Resume',
    'Country / region',
    'Scan history',
    'Next page',
    'Observed',
    'Unknown',
    'Scanning',
    'Paused',
    'Complete',
    'Cancelled',
    'Failed',
    'All countries',
    'The outer connection changed. Start a new scan.',
    'Check your WARP account and MASQUE connection, then retry.',
    'Countries are Cloudflare observations from a separate scan identity. Your configuration is checked again when connected.',
    "A complete scan can take a while. Progress is saved; resume it manually.",
    "Select an account with a saved MASQUE identity first.",
    "The temporary MASQUE connection could not be established. Check the outer connection settings.",
    "Cloudflare WARP registration failed. The code below identifies the failed stage.",
    "One port per IP: rotate 2408/500/1701/4500 between IPs. A single-IP scan uses 2408.",
    "The scan rules changed. Start a new scan; previous results are kept.",
  ],
  'zh_CN': [
    '生成 WARP 配置',
    '扫描端点',
    '快速扫描',
    '指定 IP 扫描',
    '完整 IPv4 地址池',
    '暂停',
    '继续扫描',
    '国家／地区',
    '扫描历史',
    '下一页',
    '观测时间',
    '未知',
    '扫描中',
    '已暂停',
    '已完成',
    '已取消',
    '失败',
    '全部国家／地区',
    '外层连接条件已变化，请开始新的扫描。',
    '请检查 WARP 账号和 MASQUE 连接后重试。',
    '国家信息来自独立扫描身份的 Cloudflare 观测。使用你的配置连接后会重新测量。',
    "完整扫描可能耗时较长。进度会保存，之后可手动续扫。",
    "请先选择已保存 MASQUE 凭据的账号。",
    "无法建立临时 MASQUE 连接，请检查外层连接设置。",
    "Cloudflare WARP 注册失败，下方错误码标明失败阶段。",
    "每个 IP 仅测试一个端口，按 IP 轮换 2408/500/1701/4500；指定 IP 使用 2408。",
    "扫描规则已更新，请开始新的扫描。已有结果会保留。",
  ],
  'zh_HK': [
    '產生 WARP 設定',
    '掃描端點',
    '快速掃描',
    '指定 IP 掃描',
    '完整 IPv4 位址池',
    '暫停',
    '繼續掃描',
    '國家／地區',
    '掃描記錄',
    '下一頁',
    '觀測時間',
    '未知',
    '掃描中',
    '已暫停',
    '已完成',
    '已取消',
    '失敗',
    '所有國家／地區',
    '外層連線條件已改變，請開始新的掃描。',
    '請檢查 WARP 帳戶和 MASQUE 連線後重試。',
    '國家資料來自獨立掃描身分的 Cloudflare 觀測。使用你的設定連線後會重新測量。',
    "完整掃描可能需時較長。進度會儲存，之後可手動繼續。",
    "請先選擇已儲存 MASQUE 憑證的帳戶。",
    "無法建立臨時 MASQUE 連線，請檢查外層連線設定。",
    "Cloudflare WARP 註冊失敗，下方錯誤碼標明失敗階段。",
    "每個 IP 只測試一個連接埠，按 IP 輪換 2408/500/1701/4500；指定 IP 使用 2408。",
    "掃描規則已更新，請開始新的掃描。已有結果會保留。",
  ],
  'zh_TW': [
    '產生 WARP 設定',
    '掃描端點',
    '快速掃描',
    '指定 IP 掃描',
    '完整 IPv4 位址池',
    '暫停',
    '繼續掃描',
    '國家／地區',
    '掃描紀錄',
    '下一頁',
    '觀測時間',
    '未知',
    '掃描中',
    '已暫停',
    '已完成',
    '已取消',
    '失敗',
    '所有國家／地區',
    '外層連線條件已變更，請開始新的掃描。',
    '請檢查 WARP 帳號和 MASQUE 連線後重試。',
    '國家資訊來自獨立掃描身分的 Cloudflare 觀測。使用你的設定連線後會重新測量。',
    "完整掃描可能耗時較長。進度會儲存，之後可手動繼續。",
    "請先選擇已儲存 MASQUE 憑證的帳號。",
    "無法建立暫時的 MASQUE 連線，請檢查外層連線設定。",
    "Cloudflare WARP 註冊失敗，下方錯誤碼標示失敗階段。",
    "每個 IP 僅測試一個連接埠，按 IP 輪替 2408/500/1701/4500；指定 IP 使用 2408。",
    "掃描規則已更新，請開始新的掃描。已有結果會保留。",
  ],
  'ja': [
    'WARP 設定を生成',
    'エンドポイントをスキャン',
    '簡易スキャン',
    '指定 IP をスキャン',
    'IPv4 プール全体',
    '一時停止',
    '再開',
    '国・地域',
    'スキャン履歴',
    '次のページ',
    '観測日時',
    '不明',
    'スキャン中',
    '一時停止中',
    '完了',
    'キャンセル済み',
    '失敗',
    'すべての国',
    '外側の接続が変わりました。新しいスキャンを開始してください。',
    'WARP アカウントと MASQUE 接続を確認して再試行してください。',
    '国は専用スキャン ID に対する Cloudflare の観測です。接続時に自分の設定で再測定します。',
    "完全スキャンには時間がかかる場合があります。進捗は保存され、手動で再開できます。",
    "保存済みの MASQUE 認証情報を持つアカウントを選択してください。",
    "一時的な MASQUE 接続を確立できません。外側の接続設定を確認してください。",
    "Cloudflare WARP の登録に失敗しました。下のコードで失敗した段階を確認できます。",
    "各 IP で 1 ポートのみ検査し、2408/500/1701/4500 を順に割り当てます。指定 IP は 2408 を使用します。",
    "スキャン方式が変更されました。新しいスキャンを開始してください。以前の結果は保持されます。",
  ],
  'ko': [
    'WARP 구성 생성',
    '엔드포인트 검색',
    '빠른 검색',
    '단일 IP 검색',
    '전체 IPv4 풀',
    '일시 중지',
    '계속',
    '국가 / 지역',
    '검색 기록',
    '다음 페이지',
    '관측 시간',
    '알 수 없음',
    '검색 중',
    '일시 중지됨',
    '완료',
    '취소됨',
    '실패',
    '모든 국가',
    '외부 연결이 변경되었습니다. 새 검색을 시작하세요.',
    'WARP 계정과 MASQUE 연결을 확인하고 다시 시도하세요.',
    '국가는 별도 검색 ID에 대한 Cloudflare 관측값입니다. 연결 시 사용자 구성으로 다시 측정합니다.',
    "전체 검색에는 시간이 걸릴 수 있습니다. 진행 상황을 저장하며 수동으로 계속할 수 있습니다.",
    "저장된 MASQUE 인증 정보가 있는 계정을 먼저 선택하세요.",
    "임시 MASQUE 연결을 설정할 수 없습니다. 외부 연결 설정을 확인하세요.",
    "Cloudflare WARP 등록에 실패했습니다. 아래 코드는 실패한 단계를 나타냅니다.",
    "IP마다 포트 하나만 검사하며 2408/500/1701/4500을 번갈아 배정합니다. 단일 IP 검색은 2408을 사용합니다.",
    "검색 규칙이 변경되었습니다. 새 검색을 시작하세요. 기존 결과는 유지됩니다.",
  ],
  'de': [
    'WARP-Konfiguration erstellen',
    'Endpunkte suchen',
    'Schnellsuche',
    'Eine IP prüfen',
    'Gesamter IPv4-Pool',
    'Pausieren',
    'Fortsetzen',
    'Land / Region',
    'Suchverlauf',
    'Nächste Seite',
    'Beobachtet',
    'Unbekannt',
    'Suche läuft',
    'Pausiert',
    'Abgeschlossen',
    'Abgebrochen',
    'Fehlgeschlagen',
    'Alle Länder',
    'Die äußere Verbindung hat sich geändert. Neue Suche starten.',
    'WARP-Konto und MASQUE-Verbindung prüfen und erneut versuchen.',
    'Länder sind Cloudflare-Beobachtungen einer separaten Scan-Identität. Beim Verbinden wird Ihre Konfiguration erneut geprüft.',
    "Ein vollständiger Scan kann länger dauern. Der Fortschritt wird gespeichert; manuell fortsetzen.",
    "Wähle zuerst ein Konto mit gespeicherten MASQUE-Zugangsdaten.",
    "Die temporäre MASQUE-Verbindung konnte nicht hergestellt werden. Prüfe die Einstellungen der äußeren Verbindung.",
    "Die Cloudflare-WARP-Registrierung ist fehlgeschlagen. Der folgende Code zeigt den betroffenen Schritt.",
    "Ein Port pro IP: 2408/500/1701/4500 werden abwechselnd zugewiesen. Eine einzelne IP wird auf 2408 geprüft.",
    "Die Scan-Regeln haben sich geändert. Starte einen neuen Scan; bisherige Ergebnisse bleiben erhalten.",
  ],
  'fr': [
    'Créer une configuration WARP',
    'Rechercher des points de terminaison',
    'Recherche rapide',
    'Analyser une IP',
    'Pool IPv4 complet',
    'Pause',
    'Reprendre',
    'Pays / région',
    'Historique',
    'Page suivante',
    'Observation',
    'Inconnu',
    'Recherche en cours',
    'En pause',
    'Terminé',
    'Annulé',
    'Échec',
    'Tous les pays',
    'La connexion externe a changé. Lancez une nouvelle recherche.',
    'Vérifiez votre compte WARP et la connexion MASQUE, puis réessayez.',
    'Les pays sont observés par Cloudflare avec une identité dédiée. Votre configuration est vérifiée à la connexion.',
    "Un scan complet peut prendre du temps. La progression est enregistrée ; reprenez manuellement.",
    "Sélectionnez un compte avec des identifiants MASQUE enregistrés.",
    "Impossible d’établir la connexion MASQUE temporaire. Vérifiez les paramètres de la connexion externe.",
    "L’inscription Cloudflare WARP a échoué. Le code ci-dessous indique l’étape en échec.",
    "Un port par IP : 2408/500/1701/4500 sont attribués à tour de rôle. Le scan d’une seule IP utilise 2408.",
    "Les règles du scan ont changé. Lancez un nouveau scan ; les résultats précédents sont conservés.",
  ],
  'es': [
    'Generar configuración WARP',
    'Buscar endpoints',
    'Búsqueda rápida',
    'Analizar una IP',
    'Todo el grupo IPv4',
    'Pausar',
    'Reanudar',
    'País / región',
    'Historial',
    'Página siguiente',
    'Observado',
    'Desconocido',
    'Buscando',
    'En pausa',
    'Completado',
    'Cancelado',
    'Error',
    'Todos los países',
    'La conexión exterior cambió. Inicia una nueva búsqueda.',
    'Comprueba la cuenta WARP y la conexión MASQUE e inténtalo de nuevo.',
    'Los países son observaciones de Cloudflare con una identidad de búsqueda separada. Tu configuración se comprueba al conectar.',
    "Un análisis completo puede tardar. El progreso se guarda; reanúdalo manualmente.",
    "Selecciona una cuenta con credenciales MASQUE guardadas.",
    "No se pudo establecer la conexión MASQUE temporal. Revisa la configuración de la conexión externa.",
    "Falló el registro de Cloudflare WARP. El código siguiente indica la etapa que falló.",
    "Un puerto por IP: se alternan 2408/500/1701/4500 entre las IP. El análisis de una sola IP usa 2408.",
    "Las reglas del análisis han cambiado. Inicia uno nuevo; se conservan los resultados anteriores.",
  ],
  'pt': [
    'Gerar configuração WARP',
    'Procurar endpoints',
    'Pesquisa rápida',
    'Analisar um IP',
    'Todo o conjunto IPv4',
    'Pausar',
    'Retomar',
    'País / região',
    'Histórico',
    'Próxima página',
    'Observado',
    'Desconhecido',
    'Pesquisando',
    'Pausado',
    'Concluído',
    'Cancelado',
    'Falhou',
    'Todos os países',
    'A ligação externa mudou. Inicie uma nova pesquisa.',
    'Verifique a conta WARP e a ligação MASQUE e tente novamente.',
    'Os países são observações da Cloudflare com uma identidade de pesquisa separada. A configuração é verificada ao ligar.',
    "Uma varredura completa pode demorar. O progresso é salvo; retome manualmente.",
    "Selecione uma conta com credenciais MASQUE salvas.",
    "Não foi possível estabelecer a conexão MASQUE temporária. Verifique as configurações da conexão externa.",
    "O registro do Cloudflare WARP falhou. O código abaixo indica a etapa que falhou.",
    "Uma porta por IP: 2408/500/1701/4500 são alternadas entre os IPs. A varredura de um único IP usa 2408.",
    "As regras da varredura mudaram. Inicie uma nova; os resultados anteriores são mantidos.",
  ],
  'nl': [
    'WARP-configuratie maken',
    'Eindpunten zoeken',
    'Snel zoeken',
    'Eén IP scannen',
    'Volledige IPv4-pool',
    'Pauzeren',
    'Hervatten',
    'Land / regio',
    'Scangeschiedenis',
    'Volgende pagina',
    'Waargenomen',
    'Onbekend',
    'Bezig met zoeken',
    'Gepauzeerd',
    'Voltooid',
    'Geannuleerd',
    'Mislukt',
    'Alle landen',
    'De buitenste verbinding is gewijzigd. Start een nieuwe scan.',
    'Controleer uw WARP-account en MASQUE-verbinding en probeer opnieuw.',
    'Landen zijn Cloudflare-waarnemingen met een aparte scanidentiteit. Uw configuratie wordt bij verbinding opnieuw gemeten.',
    "Een volledige scan kan lang duren. De voortgang wordt opgeslagen; hervat handmatig.",
    "Selecteer eerst een account met opgeslagen MASQUE-inloggegevens.",
    "De tijdelijke MASQUE-verbinding kon niet worden gemaakt. Controleer de instellingen van de buitenste verbinding.",
    "Cloudflare WARP-registratie is mislukt. De onderstaande code geeft de mislukte stap aan.",
    "Eén poort per IP: 2408/500/1701/4500 worden om beurten toegewezen. Een scan van één IP gebruikt 2408.",
    "De scanregels zijn gewijzigd. Start een nieuwe scan; eerdere resultaten blijven bewaard.",
  ],
  'it': [
    'Genera configurazione WARP',
    'Cerca endpoint',
    'Scansione rapida',
    'Scansiona un IP',
    'Intero pool IPv4',
    'Pausa',
    'Riprendi',
    'Paese / regione',
    'Cronologia',
    'Pagina successiva',
    'Osservato',
    'Sconosciuto',
    'Scansione in corso',
    'In pausa',
    'Completato',
    'Annullato',
    'Non riuscito',
    'Tutti i paesi',
    'La connessione esterna è cambiata. Avvia una nuova scansione.',
    'Controlla account WARP e connessione MASQUE, poi riprova.',
    'I paesi sono osservazioni Cloudflare con una identità di scansione separata. La configurazione viene verificata alla connessione.',
    "Una scansione completa può richiedere tempo. I progressi vengono salvati; riprendi manualmente.",
    "Seleziona un account con credenziali MASQUE salvate.",
    "Impossibile stabilire la connessione MASQUE temporanea. Controlla le impostazioni della connessione esterna.",
    "La registrazione Cloudflare WARP non è riuscita. Il codice seguente indica la fase non riuscita.",
    "Una porta per IP: 2408/500/1701/4500 sono assegnate a rotazione. La scansione di un solo IP usa 2408.",
    "Le regole di scansione sono cambiate. Avvia una nuova scansione; i risultati precedenti restano disponibili.",
  ],
  'ru': [
    'Создать конфигурацию WARP',
    'Поиск узлов',
    'Быстрое сканирование',
    'Сканировать один IP',
    'Весь пул IPv4',
    'Пауза',
    'Продолжить',
    'Страна / регион',
    'История',
    'Следующая страница',
    'Время проверки',
    'Неизвестно',
    'Сканирование',
    'Приостановлено',
    'Завершено',
    'Отменено',
    'Ошибка',
    'Все страны',
    'Внешнее соединение изменилось. Начните новое сканирование.',
    'Проверьте учётную запись WARP и соединение MASQUE и повторите попытку.',
    'Страны определяются Cloudflare для отдельной учётной записи сканирования. Ваша конфигурация проверяется при подключении.',
    "Полное сканирование может занять время. Прогресс сохраняется; продолжение вручную.",
    "Сначала выберите аккаунт с сохранёнными учётными данными MASQUE.",
    "Не удалось установить временное соединение MASQUE. Проверьте настройки внешнего соединения.",
    "Регистрация Cloudflare WARP не удалась. Код ниже указывает этап сбоя.",
    "Один порт на IP: 2408/500/1701/4500 назначаются по очереди. Для одного указанного IP используется 2408.",
    "Правила сканирования изменились. Начните новое сканирование; прежние результаты сохранены.",
  ],
  'uk': [
    'Створити конфігурацію WARP',
    'Пошук вузлів',
    'Швидке сканування',
    'Сканувати одну IP',
    'Увесь пул IPv4',
    'Пауза',
    'Продовжити',
    'Країна / регіон',
    'Історія',
    'Наступна сторінка',
    'Час перевірки',
    'Невідомо',
    'Сканування',
    'Призупинено',
    'Завершено',
    'Скасовано',
    'Помилка',
    'Усі країни',
    'Зовнішнє з’єднання змінилося. Почніть нове сканування.',
    'Перевірте обліковий запис WARP і з’єднання MASQUE та спробуйте знову.',
    'Країни визначає Cloudflare для окремої особи сканування. Ваша конфігурація перевіряється під час підключення.',
    "Повне сканування може тривати довго. Прогрес зберігається; продовження вручну.",
    "Спочатку виберіть обліковий запис зі збереженими даними MASQUE.",
    "Не вдалося встановити тимчасове з’єднання MASQUE. Перевірте налаштування зовнішнього з’єднання.",
    "Реєстрація Cloudflare WARP не вдалася. Код нижче вказує етап збою.",
    "Один порт на IP: 2408/500/1701/4500 призначаються по черзі. Для однієї вказаної IP використовується 2408.",
    "Правила сканування змінилися. Почніть нове сканування; попередні результати збережено.",
  ],
  'pl': [
    'Utwórz konfigurację WARP',
    'Szukaj punktów końcowych',
    'Szybkie skanowanie',
    'Skanuj jeden IP',
    'Cała pula IPv4',
    'Wstrzymaj',
    'Wznów',
    'Kraj / region',
    'Historia',
    'Następna strona',
    'Czas obserwacji',
    'Nieznany',
    'Skanowanie',
    'Wstrzymano',
    'Ukończono',
    'Anulowano',
    'Błąd',
    'Wszystkie kraje',
    'Połączenie zewnętrzne się zmieniło. Rozpocznij nowe skanowanie.',
    'Sprawdź konto WARP i połączenie MASQUE, a następnie ponów próbę.',
    'Kraje są obserwowane przez Cloudflare z osobną tożsamością skanowania. Konfiguracja jest sprawdzana przy połączeniu.',
    "Pełne skanowanie może potrwać. Postęp jest zapisywany; wznów ręcznie.",
    "Najpierw wybierz konto z zapisanymi danymi MASQUE.",
    "Nie można nawiązać tymczasowego połączenia MASQUE. Sprawdź ustawienia połączenia zewnętrznego.",
    "Rejestracja Cloudflare WARP nie powiodła się. Poniższy kod wskazuje etap błędu.",
    "Jeden port na IP: 2408/500/1701/4500 są przypisywane kolejno. Skanowanie pojedynczego IP używa 2408.",
    "Zasady skanowania uległy zmianie. Rozpocznij nowe; poprzednie wyniki zostaną zachowane.",
  ],
  'tr': [
    'WARP yapılandırması oluştur',
    'Uç noktaları tara',
    'Hızlı tarama',
    'Tek IP tara',
    'Tüm IPv4 havuzu',
    'Duraklat',
    'Sürdür',
    'Ülke / bölge',
    'Tarama geçmişi',
    'Sonraki sayfa',
    'Gözlem zamanı',
    'Bilinmiyor',
    'Taranıyor',
    'Duraklatıldı',
    'Tamamlandı',
    'İptal edildi',
    'Başarısız',
    'Tüm ülkeler',
    'Dış bağlantı değişti. Yeni bir tarama başlatın.',
    'WARP hesabını ve MASQUE bağlantısını kontrol edip yeniden deneyin.',
    'Ülkeler ayrı bir tarama kimliğiyle alınan Cloudflare gözlemleridir. Bağlanırken yapılandırmanız yeniden ölçülür.',
    "Tam tarama zaman alabilir. İlerleme kaydedilir; elle devam edin.",
    "Önce kayıtlı MASQUE kimlik bilgileri olan bir hesap seçin.",
    "Geçici MASQUE bağlantısı kurulamadı. Dış bağlantı ayarlarını kontrol edin.",
    "Cloudflare WARP kaydı başarısız oldu. Aşağıdaki kod başarısız aşamayı gösterir.",
    "Her IP için tek port: 2408/500/1701/4500 sırayla atanır. Tek IP taraması 2408 kullanır.",
    "Tarama kuralları değişti. Yeni bir tarama başlatın; önceki sonuçlar korunur.",
  ],
  'id': [
    'Buat konfigurasi WARP',
    'Pindai endpoint',
    'Pindai cepat',
    'Pindai satu IP',
    'Seluruh kumpulan IPv4',
    'Jeda',
    'Lanjutkan',
    'Negara / wilayah',
    'Riwayat pemindaian',
    'Halaman berikutnya',
    'Diamati',
    'Tidak diketahui',
    'Memindai',
    'Dijeda',
    'Selesai',
    'Dibatalkan',
    'Gagal',
    'Semua negara',
    'Koneksi luar berubah. Mulai pemindaian baru.',
    'Periksa akun WARP dan koneksi MASQUE, lalu coba lagi.',
    'Negara berasal dari pengamatan Cloudflare dengan identitas pemindaian terpisah. Konfigurasi Anda diperiksa saat tersambung.',
    "Pemindaian lengkap dapat memakan waktu. Progres disimpan; lanjutkan secara manual.",
    "Pilih akun dengan kredensial MASQUE yang tersimpan terlebih dahulu.",
    "Koneksi MASQUE sementara tidak dapat dibuat. Periksa pengaturan koneksi luar.",
    "Pendaftaran Cloudflare WARP gagal. Kode di bawah menunjukkan tahap yang gagal.",
    "Satu port per IP: 2408/500/1701/4500 digunakan bergiliran antar-IP. Pemindaian satu IP memakai 2408.",
    "Aturan pemindaian berubah. Mulai pemindaian baru; hasil sebelumnya tetap tersimpan.",
  ],
  'vi': [
    'Tạo cấu hình WARP',
    'Quét điểm cuối',
    'Quét nhanh',
    'Quét một IP',
    'Toàn bộ nhóm IPv4',
    'Tạm dừng',
    'Tiếp tục',
    'Quốc gia / khu vực',
    'Lịch sử quét',
    'Trang tiếp',
    'Thời gian đo',
    'Không rõ',
    'Đang quét',
    'Đã tạm dừng',
    'Hoàn tất',
    'Đã hủy',
    'Thất bại',
    'Mọi quốc gia',
    'Kết nối bên ngoài đã thay đổi. Hãy bắt đầu lượt quét mới.',
    'Kiểm tra tài khoản WARP và kết nối MASQUE rồi thử lại.',
    'Quốc gia là quan sát của Cloudflare qua danh tính quét riêng. Cấu hình của bạn được đo lại khi kết nối.',
    "Quét toàn bộ có thể mất nhiều thời gian. Tiến độ được lưu; tiếp tục thủ công.",
    "Trước tiên, hãy chọn tài khoản có thông tin xác thực MASQUE đã lưu.",
    "Không thể thiết lập kết nối MASQUE tạm thời. Hãy kiểm tra cài đặt kết nối ngoài.",
    "Đăng ký Cloudflare WARP thất bại. Mã bên dưới cho biết bước gặp lỗi.",
    "Mỗi IP chỉ thử một cổng, luân phiên 2408/500/1701/4500 giữa các IP. Quét một IP dùng cổng 2408.",
    "Quy tắc quét đã thay đổi. Hãy bắt đầu lượt quét mới; kết quả cũ được giữ lại.",
  ],
  'th': [
    'สร้างการตั้งค่า WARP',
    'สแกนจุดเชื่อมต่อ',
    'สแกนด่วน',
    'สแกน IP เดียว',
    'พูล IPv4 ทั้งหมด',
    'หยุดชั่วคราว',
    'ดำเนินการต่อ',
    'ประเทศ / ภูมิภาค',
    'ประวัติการสแกน',
    'หน้าถัดไป',
    'เวลาที่ตรวจสอบ',
    'ไม่ทราบ',
    'กำลังสแกน',
    'หยุดชั่วคราวแล้ว',
    'เสร็จสิ้น',
    'ยกเลิกแล้ว',
    'ล้มเหลว',
    'ทุกประเทศ',
    'การเชื่อมต่อภายนอกเปลี่ยนแล้ว โปรดเริ่มการสแกนใหม่',
    'ตรวจสอบบัญชี WARP และการเชื่อมต่อ MASQUE แล้วลองอีกครั้ง',
    'ประเทศมาจากการตรวจสอบของ Cloudflare ด้วยข้อมูลประจำตัวแยกต่างหาก ระบบจะตรวจสอบการตั้งค่าของคุณอีกครั้งเมื่อเชื่อมต่อ',
    "การสแกนทั้งหมดอาจใช้เวลานาน ระบบบันทึกความคืบหน้าไว้ให้ดำเนินการต่อด้วยตนเอง",
    "เลือกบัญชีที่มีข้อมูลรับรอง MASQUE บันทึกไว้ก่อน",
    "ไม่สามารถสร้างการเชื่อมต่อ MASQUE ชั่วคราวได้ โปรดตรวจสอบการตั้งค่าการเชื่อมต่อภายนอก",
    "ลงทะเบียน Cloudflare WARP ไม่สำเร็จ รหัสด้านล่างระบุขั้นตอนที่ล้มเหลว",
    "ทดสอบหนึ่งพอร์ตต่อ IP โดยสลับ 2408/500/1701/4500 ระหว่าง IP การสแกน IP เดียวใช้ 2408",
    "กฎการสแกนเปลี่ยนแล้ว โปรดเริ่มการสแกนใหม่ ผลลัพธ์เดิมยังคงอยู่",
  ],
  'ar': [
    'إنشاء إعداد WARP',
    'فحص نقاط الاتصال',
    'فحص سريع',
    'فحص عنوان IP واحد',
    'مجموعة IPv4 كاملة',
    'إيقاف مؤقت',
    'استئناف',
    'البلد / المنطقة',
    'سجل الفحص',
    'الصفحة التالية',
    'وقت الرصد',
    'غير معروف',
    'جارٍ الفحص',
    'متوقف مؤقتًا',
    'مكتمل',
    'ملغى',
    'فشل',
    'كل البلدان',
    'تغير الاتصال الخارجي. ابدأ فحصًا جديدًا.',
    'تحقق من حساب WARP واتصال MASQUE ثم أعد المحاولة.',
    'البلدان هي نتائج رصد Cloudflare بهوية فحص منفصلة. يعاد فحص إعدادك عند الاتصال.',
    "قد يستغرق الفحص الكامل وقتًا طويلًا. يُحفظ التقدم؛ استأنف يدويًا.",
    "اختر أولاً حساباً ببيانات اعتماد MASQUE محفوظة.",
    "تعذر إنشاء اتصال MASQUE المؤقت. تحقق من إعدادات الاتصال الخارجي.",
    "فشل تسجيل Cloudflare WARP. يحدد الرمز أدناه المرحلة التي فشلت.",
    "منفذ واحد لكل IP، مع تناوب 2408/500/1701/4500 بين العناوين. يستخدم فحص عنوان واحد المنفذ 2408.",
    "تغيّرت قواعد الفحص. ابدأ فحصًا جديدًا؛ تُحفظ النتائج السابقة.",
  ],
  'fa': [
    'ساخت پیکربندی WARP',
    'پویش نقاط اتصال',
    'پویش سریع',
    'پویش یک IP',
    'تمام مخزن IPv4',
    'مکث',
    'ادامه',
    'کشور / منطقه',
    'تاریخچهٔ پویش',
    'صفحهٔ بعد',
    'زمان مشاهده',
    'نامشخص',
    'در حال پویش',
    'متوقف شده',
    'تکمیل شد',
    'لغو شد',
    'ناموفق',
    'همهٔ کشورها',
    'اتصال بیرونی تغییر کرده است. پویش تازه‌ای آغاز کنید.',
    'حساب WARP و اتصال MASQUE را بررسی و دوباره تلاش کنید.',
    'کشورها مشاهده‌های Cloudflare با هویت پویش جداگانه هستند. پیکربندی شما هنگام اتصال دوباره سنجیده می‌شود.',
    "پویش کامل ممکن است زمان‌بر باشد. پیشرفت ذخیره می‌شود؛ دستی ادامه دهید.",
    "ابتدا حسابی با اطلاعات احراز هویت ذخیره‌شدهٔ MASQUE انتخاب کنید.",
    "اتصال موقت MASQUE برقرار نشد. تنظیمات اتصال بیرونی را بررسی کنید.",
    "ثبت‌نام Cloudflare WARP ناموفق بود. کد زیر مرحلهٔ خطا را مشخص می‌کند.",
    "یک درگاه برای هر IP؛ درگاه‌های 2408/500/1701/4500 به‌ترتیب میان IPها توزیع می‌شوند. پویش یک IP از 2408 استفاده می‌کند.",
    "قواعد پویش تغییر کرده است. پویش تازه‌ای آغاز کنید؛ نتایج قبلی حفظ می‌شوند.",
  ],
};

extension WarpStrings on AppStrings {
  String warp(String key) {
    final index = _keys.indexOf(key);
    if (index < 0) return key;
    return (kWarpCatalogs[catalogId] ?? kWarpCatalogs['en']!)[index];
  }
}
