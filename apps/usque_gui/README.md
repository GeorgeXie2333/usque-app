...
- Chain proxy uses a row of source choices, one saved-configuration list whose rows carry the saved/current/CONNECT-IP markers, and an action bar that names the pending selection and the reason a draft cannot be applied. Connection failures and missing DNS use inline warning banners. Chain copy ships in English, Simplified and Traditional Chinese; other locales fall back to English.
