...
Usque {{release_tag}} is a feature and reliability release that adds experimental L4 proxying, selectable HTTP/3 congestion control, and multilingual Windows installers, while improving network settings and cross-platform lifecycle handling.

Usque {{release_tag}} 是一个功能与可靠性版本，新增实验性 L4 代理、可选 HTTP/3 拥塞控制及多语言 Windows 安装程序，并改进网络设置与跨平台生命周期处理。

## Highlights / 更新亮点 ✨

- **Experimental L4 proxying** — Adds TCP transport over HTTP/3 CONNECT for SOCKS5, HTTP, and the bounded VPN/TUN bridge.
  <br>**实验性 L4 代理** — 为 SOCKS5、HTTP 和有界 VPN/TUN 桥接新增基于 HTTP/3 CONNECT 的 TCP 传输。
- **More congestion control options** — Choose CUBIC, Reno, BBRv2, or independent experimental BBRv3; fixes stalled BBRv2 sending.
  <br>**更多拥塞控制选项** — 可选择 CUBIC、Reno、BBRv2 或独立的实验性 BBRv3，并修复 BBRv2 发送停滞。
- **More reliable network settings** — Separates saved settings from runtime status and prevents late replies from overwriting newer state.
  <br>**更可靠的网络设置** — 区分设置保存与运行时生效状态，避免迟到的回复覆盖较新状态。
- **Windows installers in 21 languages** — Adds signed EXE installers and improves localized uninstall behavior.
  <br>**支持 21 种语言的 Windows 安装程序** — 新增签名 EXE 安装程序，并改进本地化卸载行为。
- **More complete translations** — Updates feature text, Windows tray messages, and Android notifications, Quick Settings, and shortcuts across 21 languages.
  <br>**更完整的多语言体验** — 补全 21 种语言的功能文案、Windows 托盘提示，以及 Android 通知、快捷设置和快捷方式。
...
- **Defaults stay the same.** CONNECT-IP with Auto and CUBIC remain the defaults. A saved congestion algorithm applies to the next manually started session, not a running connection.
  <br>**默认设置保持不变。** 默认仍为 CONNECT-IP、Auto 和 CUBIC。保存的拥塞算法将在下次手动启动会话时生效，不会改变正在运行的连接。
- **L4 and BBRv3 are experimental.** L4 requires explicit selection and does not support general UDP. It never silently falls back to CONNECT-IP/H2, replays established TCP connections, or turns failed traffic into direct traffic.
  <br>**L4 和 BBRv3 为实验性功能。** L4 需手动选择，不支持通用 UDP，不会静默回退至 CONNECT-IP/H2、重放已建立的 TCP 连接，或将失败流量转为直连。
- **Performance and runtime validation have limits.** Implementation changes are not measured throughput or fairness gains. Real installation, upgrade, VPN recovery, leak, and performance validation requires isolated environments; deterministic tests and package inspection do not establish those results.
  <br>**性能与运行时验证有其边界。** 实现改动不代表已测得吞吐或公平性提升。真实安装、升级、VPN 恢复、泄漏及性能验证需要隔离环境；确定性测试与软件包检查不能证明这些结果。
...
<details>
<summary>DNS privacy and L4 resolver behavior / DNS 隐私与 L4 解析行为</summary>
